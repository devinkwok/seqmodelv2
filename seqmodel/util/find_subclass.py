...
def find_subclasses(
    ...
    """Convenience function for finding subclasses of a superclass
    in various files. Note: does not work when the file calling this
    function is also a search path.

    Args:
        super_class (type): superclass of classes to find
        search_paths (typing.List[os.PathLike]): list of file paths to search.
        exclude (typing.List[type], optional): list of classes to exclude.
            Defaults to [].

    Yields:
        Iterator[type]: subclasses of super_class not in exclude.
    """
